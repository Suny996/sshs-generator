package com.scai.core.base.service.impl;
...
public abstract class BaseServiceImpl<T> implements IBaseService<T> {
    ...
    public Message update(T model) throws Exception {
        ...
        if (this.getDao().updateByPrimaryKey(model) > 0) {
            return Message.success(model);
        } else {
    ...
    public Message deleteById(String id) throws Exception {
        if (this.getDao().deleteByPrimaryKey(id) > 0) {
    ...
    public Message deleteByIds(List<String> ids) {
        int i = 0;
        for (String id : ids) {
            i += this.getDao().deleteByPrimaryKey(id);
        }
        if (i > 0) {
