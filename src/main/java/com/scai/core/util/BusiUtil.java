...
public class BusiUtil {
	...
	private static final Log logger = LogFactory.getLog(BusiUtil.class);
	...
	/**
	 * UUID是一个128位长的数字，一般用16进制表示，去掉分隔符为32字符（字母英文）。<br>
	 * 算法的核心思想是结合机器的网卡、当地时间、一个随即数来生成GUID。<br>
	...
	public static String base64Encode(byte[] bstr) {
		return new String(new BASE64Encoder().encode(bstr)).replaceAll("[\n\r]", "");
	}
	...
	public static String base64Encode(String bstr) {
		return base64Encode(bstr.getBytes());
	}
